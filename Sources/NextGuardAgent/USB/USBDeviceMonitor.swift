...
final class USBDeviceMonitor: ObservableObject {
    ...
    private func handleDeviceAdded(iterator: io_iterator_t) {
        ...
        while device != 0 {
            if let info = extractDeviceInfo(device) {
                ...
                ManagementClient.shared.reportUSBEvent(event)
                ...
                if action == .block {
                    ...
                    IncidentStoreManager.shared.addIncident(
                        policyName: "USB Device Policy",
                        action: "Block",
                        details: "USB device blocked: \(info.vendorName) \(info.productName) (\(info.deviceClass.rawValue))"
                    )
                } else {
    ...
    func evaluatePolicy(for device: USBDeviceInfo) -> DLPAction {
        ...
        case .allowWhitelisted:
            if let serial = device.serialNumber, whitelistedDevices.contains(serial) {
                return .allow
            }
            if whitelistedVendors.contains(device.vendorId) {
                return .allow
            }
            return .block
        }
    ...
    private func loadUSBPolicy() {
        ...
        guard let data = FileManager.default.contents(atPath: path),
              let config = try? JSONDecoder().decode(USBPolicyConfig.self, from: data) else {
            return
        }
