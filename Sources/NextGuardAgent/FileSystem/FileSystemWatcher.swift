...
struct FileDLPEvent: Codable {
    ...
    let policySource: String  // "server", "local", or "both"
}
...
final class FileSystemWatcher: ObservableObject {
    ...
    private let sensitiveExtensions: Set<String> = [
        ...
        "csv", "tsv", "json", "xml", "yaml", "yml", "txt", "rtf", "md", "log",
        "zip", "rar", "7z", "tar", "gz",
        "sql", "db", "sqlite", "mdb",
    ...
    func startWatching() {
        ...
        logger.info("Starting file system DLP watcher (dual-engine)")
    ...
    private func handleFileEvent(path: String, flags: UInt32) {
        ...
        guard !fileName.hasPrefix(".") || path.contains("/.Trash/") else { return }
        guard !path.contains("/Library/Caches/") || path.contains("/Library/Application Support/NextGuard") else { return }

        let eventType = classifyEvent(flags: flags)

        // Scan the file
        scanFile(at: path, eventType: eventType)
    }

    // MARK: - Dual-Engine File Scanning

    private func scanFile(at path: String, eventType: FileEventType) {
        let url = URL(fileURLWithPath: path)

        // Check file attributes
        guard let attrs = try? FileManager.default.attributesOfItem(atPath: path),
              let fileSize = attrs[.size] as? Int64, fileSize > 0 else { return }
        ...
        DispatchQueue.main.async { self.totalScanned += 1 }

        // --- Server policy engine scan ---
        let serverViolations = engine.scanContent(content, channel: .file)

        // --- Local policy engine scan ---
        let localResult = localEngine.evaluate(content: content, channel: .fileTransfer, metadata: [
            "filePath": path,
        ...
        // Merge results: take the strictest action
        var allRuleIds: [String] = serverViolations.map { $0.ruleId }
        var finalAction: DLPAction = .allow
        var policySource = "none"

        if !serverViolations.isEmpty {
            let highestSeverity = serverViolations.map { $0.severity }.max() ?? .info
            finalAction = actionForSeverity(highestSeverity)
            policySource = "server"
        }
        ...
        // No violations from either engine
        guard finalAction != .allow else { return }
        ...
        if localResult.action != .allow {
            localEngine.reportIncident(
                ...
                metadata: ["filePath": path, "fileName": url.lastPathComponent, "hash": hash]
            )
        }

        if finalAction == DLPAction.block {
            DispatchQueue.main.async { self.totalBlocked += 1 }
            blockFileAccess(at: path)
        } else if finalAction == DLPAction.quarantine {
            quarantineFile(at: path)
        }

        logger.warning("File DLP [\(policySource)]: \(finalAction.rawValue) \(url.lastPathComponent) rules=\(allRuleIds)")
    }
    ...
    func classifyFile(at path: String) -> FileClassification {
        ...
        if ["pem", "key", "crt", "cer", "p12", "pfx", "env"].contains(ext) { return .confidential }
        if ["sql", "db", "sqlite", "mdb"].contains(ext) { return .restricted }
    ...
    var stats: (scanned: Int, blocked: Int) {
        (totalScanned, totalBlocked)
    }
}
