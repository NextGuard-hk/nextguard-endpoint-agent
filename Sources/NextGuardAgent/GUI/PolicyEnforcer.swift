...
class PolicyEnforcer {
    ...
    func enforcePolicy(
        ...
        // Update GUI counters
        let guiAction: PolicyAction = result.action == .block ? .block :
    ...
    func syncPoliciesToGUI(from engine: DLPPolicyEngine) {
        ...
        let guiRules: [PolicyRule] = engineRules.map { rule in
            PolicyRule(
    ...
    private func isDefaultPolicies() -> Bool {
        // Check if current GUI policies are the default demo ones
        let defaultNames = ["Credit Card Data", "Personal Identifiable Information",
                           "Confidential Documents", "Source Code"]
