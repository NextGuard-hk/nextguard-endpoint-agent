...
final class EmailMonitor: @unchecked Sendable {
    ...
    func startMonitoring(config: [String: Any]? = nil) {
        queue.async { [weak self] in
            ...
            self.logger.info("EmailMonitor started (dual-engine) - SMTP proxy, WebMail, Mail.app hooks active")
        }
    }
    ...
    private func processSMTPMessage(data: Data, connectionId: UUID) {
        scanQueue.async { [weak self] in
            ...
            case .block:
                self.totalBlocked += 1
                self.logger.warning("Email BLOCKED [\(result.policySource)]: \(envelope.subject) to \(envelope.recipients.joined(separator: ","))")
        ...
        for part in parts {
            if part.contains("Content-Disposition: attachment") || part.contains("filename=") {
                ...
                attachments.append(EmailAttachment(
                    filename: filename, mimeType: mimeType, size: Int64(contentData.count),
                    sha256Hash: hash, contentSample: contentData.prefix(4096),
    ...
    // MARK: - Content Scanning - Dual-Engine (ISO 27001 A.8.12)
    private func scanEmailContent(envelope: EmailEnvelope) -> EmailScanResult {
        ...
        let (serverAction, serverPolicyId, serverMessage) = PolicyManager.shared.evaluate(
            channel: .email,
            content: envelope.bodyPlainText,
            metadata: [
                "sender": envelope.sender,
                "recipients": envelope.recipients.joined(separator: ","),
                "subject": envelope.subject,
                "attachmentCount": "\(envelope.attachments.count)",
                "messageSize": "\(envelope.messageSize)"
            ]
        )
        if serverAction == .block {
            riskScore += 80.0
            matchedPolicies.append(serverPolicyId ?? "SERVER_POLICY")
            policySource = "server"
        }

        // --- Local policy engine evaluation ---
        let emailContent = "\(envelope.subject) \(envelope.bodyPlainText)"
        let localResult = localEngine.evaluate(
            ...
            metadata: [
                ...
                "attachmentCount": "\(envelope.attachments.count)",
                "messageSize": "\(envelope.messageSize)",
                "hasAttachments": envelope.attachments.isEmpty ? "false" : "true"
            ]
        )
        if localResult.action != .allow {
            riskScore += localResult.action == .block ? 60.0 : 30.0
            ...
            localEngine.reportIncident(
                ...
                channel: .email,
                content: "Email from \(envelope.sender): \(envelope.subject)",
        ...
        // Determine final action (strictest wins)
        let action: EmailAction
        ...
        return EmailScanResult(
            envelopeId: envelope.id, action: action, matchedPolicies: matchedPolicies,
            sensitiveDataFound: sensitiveMatches, riskScore: min(riskScore, 100.0),
            scanDurationMs: Int(elapsed / 1_000_000), timestamp: Date(),
            ...
            if let regex = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive) {
                ...
                if count > 0 {
                    matches.append(SensitiveDataMatch(
                        dataType: dataType, matchCount: count, severity: severity,
                        context: "email_body", policyId: policyId
                    ))
                }
            }
        }
    ...
    private func sendBlockNotification(envelope: EmailEnvelope, result: EmailScanResult) {
        NotificationCenter.default.post(
            name: .init("NextGuardEmailBlocked"), object: nil,
            userInfo: ["subject": envelope.subject, "risk": result.riskScore, "policySource": result.policySource]
        )
    }
    ...
    private func quarantineEmail(envelope: EmailEnvelope, result: EmailScanResult) {
        ...
        logger.info("Email quarantined [\(result.policySource)]: \(file.path)")
    }
    ...
    private func logScanResult(_ result: EmailScanResult) {
        AuditLogger.shared.log(
            ...
            severity: result.action == .block ? .critical : result.action == .quarantine ? .high : .info,
            action: "email_scan",
            description: "Email scan [\(result.policySource)]: \(result.action.rawValue) (risk: \(result.riskScore))",
            metadata: [
                "envelopeId": result.envelopeId.uuidString,
                "action": result.action.rawValue,
                "riskScore": "\(result.riskScore)",
                "policies": result.matchedPolicies.joined(separator: ","),
