...
//
// Lightweight HTTP server on 127.0.0.1:8080
...
final class BlockPageServer: @unchecked Sendable {
    ...
    // MARK: - Custom Block Message (editable)
    var customBlockMessage: String {
        get { UserDefaults.standard.string(forKey: "NGBlockPageMessage") ?? "" }
        set { UserDefaults.standard.set(newValue, forKey: "NGBlockPageMessage") }
    }
    ...
    private func handleConnection(_ connection: NWConnection) {
        ...
        connection.receive(minimumIncompleteLength: 1, maximumLength: 4096) { [weak self] data, _, isComplete, error in
            ...
            // Parse Host header from HTTP request to show which domain was blocked
